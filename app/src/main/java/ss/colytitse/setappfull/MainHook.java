...
public class MainHook implements IXposedHookLoadPackage {
    ...
    static {
        xsp = new XSharedPreferences(BuildConfig.APPLICATION_ID, "config");
        xsp.makeWorldReadable();
    }

    private String getSystemMode() {
        xsp.reload(); // Reload to get the latest values
        return xsp.getString("SystemMode", "");
    }

    private String getTimelyMode() {
        xsp.reload(); // Reload to get the latest values
        return xsp.getString("TimelyMode", "");
    }

    private boolean isScopeModeEnabled() {
        xsp.reload(); // Reload to get the latest values
        return xsp.getBoolean("scope_mode_switch", true);
    }
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        if (!lpparam.packageName.equals(lpparam.processName))
            return;
        if ((isScopeModeEnabled() || getTimelyMode().contains(lpparam.packageName)) && !lpparam.packageName.equals("android")) {
            onTimelyMode();
            return;
        }
